...
def send_slack_alert(message_to_send, webhook_url):
    ...
    req = Request(url=webhook_url, data=json.dumps(payload).encode(), headers={'Content-Type': 'application/json'}, method='POST')
...
def lambda_handler(event, context):
    ...
    if "RUNNING" in task_state_detail["desiredStatus"]:
        return
    if "STOPPED" in task_state_detail["desiredStatus"]:
        ...
        for i in root_servers:
            ...
            lsi_object = datetime.strptime(i["last_successful_import"], "%Y-%m-%dT%H:%M:%S.%f%z")
